...
print("Evaluation results for XGBoost regression")

#Store evaluation values in text file
metrics = open("eval_XGBoost_Metrics.txt", "w")
...
for fileCounter in range(totalFiles):
    ...
    while i <= rowsData:
        ...
        # create an XGBoost model with a linear kernel
        ...
        xgboost.fit(trainDataX, trainDataY)
        ...
        predicted_xgboost_Self = xgboost.predict(testDataX)
        # predicted_knn_Self = knn_regressor.predict(testDataX)
        predictedSelf = float(predicted_xgboost_Self[0])
    ...
    resultsFileName = 'eval_XGBoost_Results_'+ fileName
