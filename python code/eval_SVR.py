...
print("Evaluation results for support vector regression")

#Store evaluation values in text file
metrics = open("eval_SVR_Metrics.txt", "w")
...
for fileCounter in range(totalFiles):
    ...
    while i <= rowsData:
        ...
        predicted_svr_Self = svr.predict(testDataX)
        #predicted_knn_Self = knn_regressor.predict(testDataX)
        predictedSelf = float(predicted_svr_Self[0])
    ...
    resultsFileName = 'eval_SVR_Results_'+ fileName
