...
print("Evaluation results for decision tree regression")

#Store evaluation values in text file
metrics = open("eval_DTR_Metrics.txt", "w")
...
for fileCounter in range(totalFiles):
    ...
    while i <= rowsData:
        ...
        regressor.fit(trainDataX, trainDataY)
        ...
        predicted_dtr_Self = regressor.predict(testDataX)
        predictedSelf = float(predicted_dtr_Self[0])
    ...
    resultsFileName = 'eval_DTR_Results_'+ fileName
